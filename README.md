...
- Text Editor: A text editor of your choice, used for editing the code/config file. If you don't know which one to use, I recommend using [Visual Studio Code](https://code.visualstudio.com/).
...
> **Note:** The ``config.toml`` isn't updated by default when updating the bot files, you are required to check if any changes happened to the ``config.toml`` file by yourself, it's your responsibility.
...
1. **Open Issues:** Found a bug or have a feature to request? Open an issue to let us know. This helps us identify and fix problems or consider new features.
